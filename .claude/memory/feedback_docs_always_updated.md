...
**How to apply:** After every code change, proactively update the relevant docs: `README.md` for features/config, inline docstrings for logic changes, `.claude/CLAUDE.md` for architectural changes, and any other relevant `docs/` files. Run `just sync-copilot` after any CLAUDE.md change. Do not wait to be asked — treat doc updates as part of the task completion criteria.
